...
if(isset($_POST['update_profile'])){
   ...
   mysqli_query($conn, "UPDATE `user_form` SET nome_cli = '$update_name', email = '$update_email' WHERE iduser = '$user_id'") or die('query failed');
   ...
   if(!empty($update_pass) || !empty($new_pass) || !empty($confirm_pass)){
      if($update_pass != $old_pass){
         ...
         mysqli_query($conn, "UPDATE `user_form` SET password = '$confirm_pass' WHERE iduser = '$user_id'") or die('query failed');
   ...
   if(!empty($update_image)){
      if($update_image_size > 2000000){
         ...
         $image_update_query = mysqli_query($conn, "UPDATE `user_form` SET image = '$update_image' WHERE iduser = '$user_id'") or die('query failed');
...
<div class="update-profile">
   ...
   <?php
      $select = mysqli_query($conn, "SELECT * FROM `user_form` WHERE iduser = '$user_id'") or die('query failed');
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <div class="flex">
         <div class="inputBox">
            ...
            <span>Username :</span>
            <input type="text" name="update_name" value="<?php echo $fetch['nome_cli']; ?>" class="box">
